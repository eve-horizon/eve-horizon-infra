...
This queries the configured public ECR registry for published semver tags and compares them against the version pinned in `config/platform.yaml`.

Three outcomes:
- **"You are on the latest version"** — stop here, nothing to do
- **"Latest available: X.Y.Z"** — proceed to step 3
- **"Could not determine latest version"** — check network access, then compare `https://github.com/eve-horizon/eve-horizon/tags` with the public ECR gallery
